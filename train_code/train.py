"""CIPHER diffusion fine-tuning entry point.

This stage adapts the Stable Diffusion backbone used across the paper's four
causal intervention paths. Parts of the training stack are adapted from the
Hugging Face Diffusers DreamBooth and text-to-image example scripts.
"""
...
from dataset import CIPHERFineTuningImageDirectoryDataset
...
def main(args):
    ...
    if not args.image_dir or not args.prompt_dir:
        raise ValueError("image_dir and prompt_dir must be set for directory datasets.")

    train_dataset = CIPHERFineTuningImageDirectoryDataset(
