...
def parse_args():
    ...
    parser.add_argument(
        "--imagenet_init",
        action="store_true",
        help="Initialize backbone with ImageNet weights before checkpoint",
    )
...
def main():
    ...
    transform = build_transforms(TRANSFORM_IMAGENET, is_train=False)
    ...
    for csv_path in args.csv:
        ...
        print(
            f"Detected {ds.num_labels} label columns in {csv_path} ->"
            f" {ds.num_labels} classifier heads"
        )
        datasets.append(ds)
    ...
    header = (
        ...
        "backbone=DenseNet-121 (torchvision)",
    )
