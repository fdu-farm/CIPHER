...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train DenseNet-121 multi-head classifier from scratch (no pretraining).",
    ...
    parser.add_argument(
        "--log_dir",
        type=Path,
        default=Path("runs/multihead_densenet"),
...
def main():
    ...
    train_transform = build_transforms(TRANSFORM_MEDICAL, is_train=True)
    val_transform = build_transforms(TRANSFORM_MEDICAL, is_train=False)
    ...
    if best_overall_path:
        print(
            "Best validation loss across all runs:"
            f" {best_overall_loss:.4f} saved at {best_overall_path}"
        )
            ...
            f.write(
                "\n".join(
                    [
                        ...
                        "  backbone: DenseNet-121 (random init)",
                        f"  batch_size: {bs}",
                        f"  lr: {lr}",
                        f"  epoch: {epoch}",
