"""CIPHER Path 3/4 batch counterfactual editor.

This runner produces instance-anchored matched pairs by reusing the inversion
trajectory while swapping either subgroup metadata tokens (Path 3) or disease
tokens (Path 4) in the edited prompt.
"""
...
DEFAULT_OUTPUT_DIR = "outputs/nulltext_batch"
...
def default_output_name(image_path: Path, index: int) -> str:
    stem = image_path.stem if image_path else "image"
    return f"{stem}_{index:05d}.png"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Batch image+prompt generation using Null-Text inversion.",
        parents=[config_parser],
    )
    parser.add_argument("--csv", required=True, help="CSV file with image and prompt columns.")
    ...
    parser.add_argument(
        "--inversion-prompt",
        default=None,
        help="Prompt used for inversion; defaults to per-row prompt.",
    )
    parser.add_argument(
        "--guidance-scale",
        type=float,
...
def main() -> None:
    ...
    if "image" not in rows[0] and "input_image" not in rows[0]:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if is_blank(image_value):
            raise ValueError(f"Row {idx} missing image path.")
        image_path = resolve_path(str(image_value), base_dir)
        prompt = row.get("prompt")
        if is_blank(prompt):
            prompt = args.prompt

        inversion_prompt = row.get("inversion_prompt")
        if is_blank(inversion_prompt):
            inversion_prompt = args.inversion_prompt or prompt
        ...
        null_text_steps = parse_optional_int(row.get("null_text_steps"), args.null_text_steps)
        null_text_lr = parse_optional_float(row.get("null_text_lr"), args.null_text_lr)
        skip_steps = parse_optional_int(row.get("skip_steps"), args.skip_steps)
        ...
        row_edit_target = row.get("edit_target", args.edit_target)
        if isinstance(row_edit_target, str):
            row_edit_target = row_edit_target.strip().lower()
        if row_edit_target not in {"lesion", "normal", "all"}:
            row_edit_target = args.edit_target
        if seed is not None:
            torch.manual_seed(seed)

        scheduler_name = row.get("scheduler") if not is_blank(row.get("scheduler")) else args.scheduler
        ddim_scheduler = build_scheduler("ddim", pipeline)
        ddim_scheduler.set_timesteps(num_steps, device=device)

        edit_scheduler = build_scheduler(scheduler_name, pipeline)
        edit_scheduler.set_timesteps(num_steps, device=device)
        use_ddim_step = isinstance(edit_scheduler, DDIMScheduler)
        if not use_ddim_step:
            print(
                f"Warning: using {scheduler_name} for editing while inversion/optimization uses DDIM; "
        ...
        if seg_model is not None:
            ...
            if row_edit_target == "normal":
                raw_mask = 1.0 - raw_mask
            elif row_edit_target == "all":
                raw_mask = torch.ones_like(raw_mask)
            coverage = raw_mask.mean().item()
            if coverage < 1e-5:
                print(f"Warning: segmentation mask is empty for row {idx}; editing the full image instead.")
                raw_mask = torch.ones_like(raw_mask)
        ...
        skip_steps = max(0, min(skip_steps, len(edit_scheduler.timesteps) - 1))
        ...
        output_name = default_output_name(image_path, idx)
